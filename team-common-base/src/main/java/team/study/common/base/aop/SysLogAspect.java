...
import team.study.common.base.context.ThreadLocalParam;
import team.study.common.base.event.SysLogEvent;
...
import team.study.common.base.utils.log.LogUtil;
...
public class SysLogAspect {
     ...
     * 标记了@SysLog 注解的方法
     */
    @Pointcut("@annotation(team.study.common.base.aop.SysLog)")
        ...
        tryCatch(p -> {
            ...
            if (r == null) {
                ...
                if (r.getIsSuccess()) {
                    ...
                    sysLogDTO.setExDetail(r.getErrMessage());
                }
                if (sysLog.response()) {
    ...
    private String getArgs(Object[] args, HttpServletRequest request) {
        ...
        try {
            if (!request.getContentType().contains(FORM_DATA_CONTENT_TYPE)) {
                strArgs = JsonUtil.beanToJson(params);
            }
        } catch (Exception e) {
            try {
