package team.study.common.jwt.utils;
...
import team.study.common.base.utils.DateUtil;
import team.study.common.base.utils.StrPool;
import team.study.common.jwt.model.Token;
...
import static team.study.common.base.context.ContextConstants.BASIC_HEADER_PREFIX;
import static team.study.common.base.enums.ExceptionCode.JWT_BASIC_INVALID;
import static team.study.common.base.enums.ExceptionCode.JWT_PARSER_TOKEN_FAIL;
...
public final class JwtUtil {
    ...
    public static String[] getClient(String basicHeader) {
        if (StrUtil.isEmpty(basicHeader) || !basicHeader.startsWith(BASIC_HEADER_PREFIX)) {
            throw ExceptionFactory.bizException(JWT_BASIC_INVALID);
        }
    ...
    public static Claims parseJwt(String jsonWebToken, long allowedClockSkewSeconds) {
        try {
            ...
            //过期
            throw ExceptionFactory.authException(ExceptionCode.JWT_TOKEN_EXPIRED);
        } catch (SignatureException ex) {
            log.error("token=[{}] 签名错误", jsonWebToken, ex);
            //签名错误
            throw ExceptionFactory.authException(ExceptionCode.JWT_SIGNATURE);
        } catch (IllegalArgumentException ex) {
            log.error("token=[{}] 为空", jsonWebToken, ex);
            //token 为空
            throw ExceptionFactory.authException(ExceptionCode.JWT_ILLEGAL_ARGUMENT);
        } catch (Exception e) {
            log.error("token=[{}] errCode:{}, message:{}", jsonWebToken, JWT_PARSER_TOKEN_FAIL.getCode(), e.getMessage(), e);
            throw ExceptionFactory.authException(JWT_PARSER_TOKEN_FAIL);
        }
    }

    public static String getToken(String token) {
        if (token == null) {
            throw ExceptionFactory.bizException(JWT_PARSER_TOKEN_FAIL);
        }
